...
上の3点に、変更してよい範囲と確認方法を足すと、AIへの依頼にも使えます。

```markdown
## 作業範囲
- profiles/taro.mdだけを追加する
- テンプレートと既存プロフィールは変更しない

## 確認方法
- 必須項目とプレースホルダーの残りを確認する
- PRのFiles changedで変更対象が1ファイルであることを確認する
```

機能開発なら「何を入力したら何が起きるか」を完了条件に書きます。「いい感じに直す」だけで範囲が広がる場合は、まず調査と修正案を依頼します。小さな修正では、毎回長い仕様書を作る必要はありません。
...
まず現在の作業状態を確認し、未コミット変更がなければmainへ移動します。変更がある場合は、その作業のbranchでcommitするか退避方針を決めてから進みます。
